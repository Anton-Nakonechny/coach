...
Task statement: Distribute tools appropriately across agents and configure tool choice.
Frame questions in these exam scenarios: Multi-Agent Research System; Structured Data Extraction.

Test knowledge of:
- That giving an agent too many tools (e.g., 18 instead of 4-5) degrades tool selection reliability by increasing decision complexity.
...
- Using forced tool selection to ensure a specific tool is called first (e.g., forcing extract_metadata before enrichment tools), then processing subsequent steps in follow-up turns.
